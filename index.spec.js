...
describe("SauceDemo UI Automation Tests", () => {
  ...
  it("should add items to the cart and complete checkout", async () => {
    ...
    // Sort the items
    await driver.wait(
      until.elementLocated(By.css(".product_sort_container")),
      10000
    );
    await driver
      .findElement(By.css(".product_sort_container"))
      .sendKeys("lohi", Key.RETURN);

    // Add two or more items to the shopping cart
    const itemElements = await driver.findElements(
      By.css(".inventory_item_name")
    );
    const itemNames = await Promise.all(
      itemElements.map((element) => element.getText())
    );
    ...
    const cartItemElements = await driver.findElements(
      By.xpath('//div[@class="cart_item_label"]/a')
    );
    const cartItemNames = await Promise.all(
      cartItemElements.map((element) => element.getText())
    );
    const itemFound = itemsToAdd.some((itemName) =>
      cartItemNames.includes(itemName)
    );
    console.log("Items added to cart:", cartItemNames);
    console.log("Expected items:", itemsToAdd);
    assert(
      itemFound,
      `Expected items to be in the cart, but the actual items are: ${cartItemNames.join(
        ", "
      )}`
    );
    ...
    const removeButton = await driver.findElement(
      By.css(".cart_item .btn_secondary")
    );
    await removeButton.click();
    await driver.navigate().back();

    // Add another item
    const backpackButton = await driver.findElement(
      By.xpath(
        '//div[contains(text(), "Sauce Labs Backpack")]/ancestor::div[@class="inventory_item"]//button'
      )
    );
    ...
    try {
      ...
      const isCheckoutComplete = await driver
        .wait(until.elementLocated(By.css(".complete-header")), 10000)
        .isDisplayed();
    ...
    const purchasedItemElements = await driver.findElements(
      By.css(".inventory_item_name")
    );
    const purchasedItemNames = await Promise.all(
      purchasedItemElements.map((element) => element.getText())
    );
    ...
    assert(
      itemFound2,
      `Expected '${expectedItem}' to be in the purchased items, but the purchased items are: ${purchasedItemNames.join(
        ", "
      )}`
    );
    ...
    const totalPrice = await driver
      .findElement(By.css(".summary_total_label"))
      .getText();
